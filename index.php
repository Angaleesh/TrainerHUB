...
if (isset($_POST['login_user'])) {
  ...
  if (count($errors) == 0) {
    if ($user == 'login') {
      $query = "SELECT * FROM $user WHERE uname='$username' AND pwd='$pwd'";
      $results = mysqli_query($conn, $query);
      if (mysqli_num_rows($results) == 1) {
        $_SESSION['uname'] = $username;
        header("location:home.php?info=member_serach");
      } else {
      ...
      if (mysqli_num_rows($results) == 1) {
        $_SESSION['uname'] = $username;
        header("location:t_home.php?info=view_profile");
      } else {
        array_push($errors, "<div class='alert alert-warning'><b>Wrong username/password combination</b></div>");
      }
    }
      ...
      if (mysqli_num_rows($results) == 1) {
        $_SESSION['uname'] = $username;
        header("location:m_home.php?info=profile");
      } else {
        array_push($errors, "<div class='alert alert-warning'><b>Wrong username/password combination</b></div>");
      }
    }
  ...
  <nav class="navbar navbar-expand-md bg-dark navbar-dark">
    <div class="container-fluid">
      <a class="navbar-brand" href="index.php">
        <h3 style="color:white; text-align:center;">GYM MANAGEMENT SYSTEM</h3>
      </a>

    </div>
  </nav>
  <hr>
  <h2 style="color:white; text-align:center;"> Make It Easier</h2>
  <hr>
